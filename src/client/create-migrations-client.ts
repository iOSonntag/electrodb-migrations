...
import type { InternalEntityOptions } from '../internal-entities/index.js';
import { readLockRow } from '../lock/index.js';
import { applyBatch, finalizeFlow, loadPendingMigrations } from '../runner/index.js';
import type { HistoryRow, ItemCounts, RawHistoryRow } from '../runner/index.js';
 ...
 *
 * **Two clients in play:**
 * - `docClient` (unguarded) — used by the runner for v1 scan, v2 write,
 *   _migrations / _migration_state / _migration_runs reads/writes. The runner
 *   MUST be unguarded — otherwise it would gate itself.
...
export function createMigrationsClient(args: CreateMigrationsClientArgs): MigrationsClient {
  ...
  // Step 2: ensure DocumentClient. Service wrapper accepts DocumentClient only.
  const docClient =
    args.client instanceof DynamoDBClient
      ? DynamoDBDocumentClient.from(args.client)
      : (args.client as DynamoDBDocumentClient);

  // Step 3: build internal-entity bundle for the runner.
  const internalOptions: InternalEntityOptions | undefined =
    args.config.keyNames.electroEntity !== undefined || args.config.keyNames.electroVersion !== undefined
      ? {
          identifiers: {
            ...(args.config.keyNames.electroEntity !== undefined ? { entity: args.config.keyNames.electroEntity } : {}),
            ...(args.config.keyNames.electroVersion !== undefined ? { version: args.config.keyNames.electroVersion } : {}),
          },
        }
      : undefined;
  ...
  // wrapClient mutates the client in place and returns it.
  ...
  const client: MigrationsClient = {
    ...
    async finalize(arg) {
      if (typeof arg === 'string') {
        ...
        return { finalized: [{ migId: arg, itemCounts: result.itemCounts as ItemCounts }] };
      }
      ...
      for (const row of applied) {
        ...
        finalized.push({ migId: row.id, itemCounts: result.itemCounts as ItemCounts });
      }
      return { finalized };
    },
...
function resolveTableName(args: CreateMigrationsClientArgs): string {
  if (args.tableName !== undefined && args.tableName.length > 0) return args.tableName;
